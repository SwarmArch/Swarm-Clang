...
//===----------------------------------------------------------------------===//
// Swarm spawn, Swarm for
//===----------------------------------------------------------------------===//

void ASTStmtReader::VisitSwarmSpawnStmt(SwarmSpawnStmt *S) {
  VisitStmt(S);
  S->setSpawnLoc(ReadSourceLocation());
  S->setInit(Record.readSubStmt());
  S->setConditionVariable(Record.getContext(), ReadDeclAs<VarDecl>());
  S->setTimestamp(Record.readSubExpr());
  S->setSpawnedStmt(Record.readSubStmt());
}

//TODO(victory): void ASTStmtReader::VisitSwarmForStmt(SwarmForStmt *S) {
...
Stmt *ASTReader::ReadStmtFromStream(ModuleFile &F) {
  ...
  while (true) {
    ...
    case STMT_SWARMSPAWN:
      S = new (Context) SwarmSpawnStmt(Empty);
      break;

    case STMT_SWARMFOR:
      //TODO(victory): S = new (Context) SwarmForStmt(Empty);
      break;
