...
//===----------------------------------------------------------------------===//
void ASTStmtWriter::VisitSwarmSpawnStmt(SwarmSpawnStmt *S) {
  VisitStmt(S);
  Record.AddSourceLocation(S->getSpawnLoc());
  Record.AddStmt(S->getInit());
  Record.AddDeclRef(S->getConditionVariable());
  Record.AddStmt(S->getTimestamp());
  Record.AddStmt(S->getSpawnedStmt());
  Code = serialization::STMT_SWARMSPAWN;
}

//TODO(victory): void ASTStmtWriter::VisitCilkForStmt(CilkForStmt *S) {
