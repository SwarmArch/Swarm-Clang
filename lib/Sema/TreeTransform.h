...
#include "clang/Basic/OpenMPKinds.def"
  ...
  /// Subclasses may override this routine to provide different behavior.
  //TODO(victory):
  //StmtResult RebuildSwarmForStmt(SourceLocation ForLoc, SourceLocation LParenLoc,
  //                              Stmt *Init, Sema::ConditionResult Cond,
  //                              Sema::FullExprArg Inc, SourceLocation RParenLoc,
  //                              Stmt *Body) {
  //  return getSema().ActOnSwarmForStmt(ForLoc, LParenLoc, Init, Cond,
  //                                     Inc, RParenLoc, Body);
  //}
  ...
  /// \brief Build a new Swarm spawn statment.
  ///
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  StmtResult RebuildSwarmSpawnStmt(SourceLocation SpawnLoc,
                                   Stmt *Init,
                                   Sema::ConditionResult Cond,
                                   Stmt *S,
                                   bool isSub, bool isSuper) {
    return getSema().ActOnSwarmSpawnStmt(SpawnLoc,
                                         Init, Cond, S,
                                         isSub, isSuper);
  }
  StmtResult RebuildSwarmSpawnStmt(SourceLocation SpawnLoc,
                                   Stmt *S) {
    return getSema().ActOnSwarmSpawnStmt(SpawnLoc, S);
  }
...
template<typename Derived>
StmtResult
TreeTransform<Derived>::TransformSwarmSpawnStmt(SwarmSpawnStmt *S) {
  // Transform the body of the switch statement.
  StmtResult SpawnedStmt = getDerived().TransformStmt(S->getSpawnedStmt());
  if (SpawnedStmt.isInvalid())
    return StmtError();
  ...
  if (S->getTimestamp()) {
    ...
    // Transform the condition.
    Sema::ConditionResult Cond = getDerived().TransformCondition(
        S->getSpawnLoc(), S->getConditionVariable(), S->getTimestamp(),
        Sema::ConditionKind::Switch);
    if (Cond.isInvalid())
      return StmtError();

    return getDerived().RebuildSwarmSpawnStmt(
        S->getSpawnLoc(), Init.get(), Cond, SpawnedStmt.get(), S->isSubdomain(),
        S->isSuperdomain());
  } else {
    return getDerived().RebuildSwarmSpawnStmt(S->getSpawnLoc(),
                                              SpawnedStmt.get());
  }
}
