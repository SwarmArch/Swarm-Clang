...
struct LoopAttributes {
  ...
  /// \brief Value for llvm.loop.swarmify.enable metadata.
  LVEnableState SwarmifyEnable;

  /// \brief Value for llvm.loop.expand.enable metadata.
  LVEnableState ExpandEnable;

  /// \brief Value for llvm.loop.coarsen.factor metadata.
  unsigned CoarsenFactor;
};
...
public:
  ...
  /// \brief Set the next pushed loop 'swarmify.enable'
  void setSwarmifyEnable(bool Enable = true) {
    StagedAttrs.SwarmifyEnable =
        Enable ? LoopAttributes::Enable : LoopAttributes::Disable;
  }

  /// \brief Set the next pushed loop 'expand.enable'
  void setExpandEnable(bool Enable = true) {
    StagedAttrs.ExpandEnable =
        Enable ? LoopAttributes::Enable : LoopAttributes::Disable;
  }

  /// \brief Set the coarsen factor for the next loop pushed.
  void setCoarsenFactor(unsigned W) { StagedAttrs.CoarsenFactor = W; }
