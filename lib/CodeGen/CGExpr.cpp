...
/// EvaluateExprAsInt - Perform the usual unary conversions on the specified
/// expression and return an integer-type value of the specified width.
llvm::Value *CodeGenFunction::EvaluateExprAsInt(const Expr *E, const unsigned d) {
  PGO.setCurrentStmt(E);
  if (const MemberPointerType *MPT = E->getType()->getAs<MemberPointerType>()) {
    assert(false && "We are not handling pointer type timestamps.");
    llvm::Value *MemPtr = EmitScalarExpr(E);
    return CGM.getCXXABI().EmitMemberPointerIsNotNull(*this, MemPtr, MPT);
  }

  //FIXME(victory): This is using size_t as a hacky standin for uint64_t.
  QualType IntTy = getContext().getSizeType();
  SourceLocation Loc = E->getExprLoc();
  if (!E->getType()->isAnyComplexType())
    return EmitScalarConversion(EmitScalarExpr(E), E->getType(), IntTy, Loc);

  return EmitComplexToScalarConversion(EmitComplexExpr(E), E->getType(), IntTy,
                                       Loc);
}
