...
void CodeGenFunction::DetachScope::StartDetach(bool isSwarm,
  ...
  if (isSwarm) {
    auto DI =
        Timestamp
            ? CGF.Builder.CreateSDetach(Timestamp, DetachedBlock, ContinueBlock)
            : CGF.Builder.CreateSDetach(DetachedBlock, ContinueBlock);
    DI->setSubdomain(isSubdomain);
    DI->setSuperdomain(isSuperdomain);
  } else {
...
void CodeGenFunction::DetachScope::FinishDetach(bool isSwarm) {
  ...
  if (isSwarm)
    CGF.Builder.CreateSReattach(ContinueBlock);
  else
