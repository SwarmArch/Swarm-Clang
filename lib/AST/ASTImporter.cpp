...
namespace clang {
  ...
  public:
    ...
    Stmt *VisitSwarmSpawnStmt(SwarmSpawnStmt *S);
    //TODO(victory): Stmt *VisitSwarmForStmt(SwarmForStmt *S);
...
Stmt *ASTNodeImporter::VisitSwarmSpawnStmt(SwarmSpawnStmt *S) {
  SourceLocation SpawnLoc = Importer.Import(S->getSpawnLoc());
  Stmt *Child = Importer.Import(S->getSpawnedStmt());
  if (!Child && S->getSpawnedStmt())
    return nullptr;
  if (S->getTimestamp()) {
    // structurally similar to VisitSwitchStmt
    Stmt *ToInit = Importer.Import(S->getInit());
    if (!ToInit && S->getInit())
      return nullptr;
    VarDecl *ToConditionVariable = nullptr;
    ...
    Expr *ToTimestamp = Importer.Import(S->getTimestamp());
    if (!ToTimestamp && S->getTimestamp())
      return nullptr;
    return new (Importer.getToContext()) SwarmSpawnStmt(
        Importer.getToContext(), SpawnLoc, ToInit, ToConditionVariable,
        ToTimestamp, Child, S->isSubdomain(), S->isSuperdomain());
  } else {
    return new (Importer.getToContext()) SwarmSpawnStmt(SpawnLoc, Child);
  }
}

//TODO(victory): Stmt *ASTNodeImporter::VisitSwarmForStmt(SwarmForStmpt *S) {
