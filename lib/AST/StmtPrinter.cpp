...
void StmtPrinter::VisitSwarmSpawnStmt(SwarmSpawnStmt *Node) {
  Indent() << (Node->isSubdomain() ? "_Swarm_spawn_sum " :
               Node->isSuperdomain() ? "_Swarm_spawn_super" :
               "_Swarm_spawn ");
  if (Node->getTimestamp()) {
    ...
    if (const DeclStmt *DS = Node->getConditionVariableDeclStmt())
      ...
      PrintExpr(Node->getTimestamp());
    OS << ") ";
  }

  // Pretty print compoundstmt bodies (very common).
  if (CompoundStmt *CS = dyn_cast<CompoundStmt>(Node->getSpawnedStmt())) {
    OS << " ";
    PrintRawCompoundStmt(CS);
    OS << "\n";
  } else {
...
//TODO(victory): void StmtPrinter::VisitSwarmForStmt(SwarmForStmt *Node) {
