...
// SwarmSpawnStmt
SwarmSpawnStmt::SwarmSpawnStmt(const ASTContext &C, SourceLocation SL,
  ...
  : Stmt(SwarmSpawnStmtClass), SpawnLoc(SL)
{
  setConditionVariable(C, Var);
  SubExprs[INIT] = init;
  SubExprs[TIMESTAMP] = Timestamp;
  SubExprs[SPAWNED] = SpawnedStmt;
  setSubdomain(isSub);
  setSuperdomain(isSuper);
}

VarDecl *SwarmSpawnStmt::getConditionVariable() const {
  if (!SubExprs[VAR])
    return nullptr;
...
void SwarmSpawnStmt::setConditionVariable(const ASTContext &C, VarDecl *V) {
  if (!V) {
    SubExprs[VAR] = nullptr;
    return;
  }
