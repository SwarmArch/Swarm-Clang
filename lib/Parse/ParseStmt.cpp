...
Retry:
  ...
  case tok::kw__Swarm_spawn:              // [Swarm] _Swarm_spawn statement
  case tok::kw__Swarm_spawn_sub:
  case tok::kw__Swarm_spawn_super:
    // if (!getLangOpts().CilkPlus) {
    //   Diag(Tok, diag::err_cilkplus_disable);
    //   SkipUntil(tok::semi);
    //   return StmtError();
    // }
    return ParseSwarmSpawnStatement(TrailingElseLoc);

  case tok::kw__Swarm_for:
    // if (!getLangOpts().CilkPlus) {
    //   Diag(Tok, diag::err_cilkplus_disable);
    //   SkipUntil(tok::semi);
    //   return StmtError();
    // }
    //TODO(victory): return ParseSwarmForStatement(TrailingElseLoc);
