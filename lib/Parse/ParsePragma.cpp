...
bool Parser::HandlePragmaLoopHint(LoopHint &Hint) {
  ...
  bool OptionSwarmify = false;
  bool OptionExpand = false;
  bool StateOption = false;
  if (OptionInfo) { // Pragma Unroll does not specify an option.
    OptionUnroll = OptionInfo->isStr("unroll");
    OptionDistribute = OptionInfo->isStr("distribute");
    OptionSwarmify = OptionInfo->isStr("swarmify");
    OptionExpand = OptionInfo->isStr("expand");
    StateOption = llvm::StringSwitch<bool>(OptionInfo->getName())
                      .Case("vectorize", true)
                      .Case("interleave", true)
                      .Default(false) ||
                  OptionUnroll || OptionDistribute || OptionSwarmify || OptionExpand;
  }

  bool AssumeSafetyArg = !OptionUnroll && !OptionDistribute && !OptionSwarmify && !OptionExpand;
...
///    'swarmify' '(' loop-hint-keyword ')'
///    'expand' '(' loop-hint-keyword ')'
///    'coarsen_factor' '(' loop-hint-value ')'
///
///  loop-hint-keyword:
///    'enable'
///    'disable'
